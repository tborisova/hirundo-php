<?php
  ...
  $userTweets = $db_operation->executeQuery("SELECT tweets.user_id as t_user_id, content, users.image_url, users.user_name from tweets join users on tweets.user_id=users.user_id where tweets.user_id in (select followee_id from follows where follower_id = :user_id) AND tweets.user_id != :user_id order by id desc",
                                array(":user_id"=>$user_id));
?>
      ...
      <section class="column-center">
        <div class="panel">
          <div class="panel-body">
            <h2 class="panel-title">Users which <?php echo $userRow['user_email']?> follows:</h2>
            <?php
